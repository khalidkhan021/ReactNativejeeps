import React, { Component } from 'react';
import { Container, Header, Content, Card,Button,List,Left,Thumbnail, Text,Item, Icon, Right ,Input,ListItem, CheckBox,Body,Spinner } from 'native-base';
import {View,Image,ScrollView,StyleSheet} from 'react-native';
// import { Container, Header, Content, Item, Input, Icon } from 'native-base';
// import { ScrollView } from 'react-native-gesture-handler';
import LinearGradient from 'react-native-linear-gradient';
import { Dropdown } from 'react-native-material-dropdown';


export default class Vehicle_3 extends Component {
constructor(props){
super(props);

this.state={
 isLoading:false
}
this.search=this.search.bind(this);
}
    
search(){
 
  console.warn("work")
 const {val}=this.state;
 if(!val){
   alert('Please fill the field ')
 }
else{
  {this.setState({isLoading:true})}
  fetch(`http://hnhtechsolutions.com/khalid/offroute/wp-json/wp/v2/product?search=${val}`)
  .then(response => response.json())
  .then(data => {
    {this.setState({isLoading:false})
    console.log('data*****',data)
  }
  data.map((i)=>{
      return this.setState({s_data : [i] },()=>{console.warn(this.state.s_data)})
  })
 
   
  })
  .catch(error => console.error(error))

}

}

    render(){
 const {s_data,isLoading}= this.state
        return(
        
        <Container style={{flex:1,backgroundColor: '#231F20'}}>
            <ScrollView>
      

        <Card style={{alignSelf:"center",width:"90%",justifyContent:"center",marginTop:"1%",backgroundColor:"#58595B",padding:10}}>

              <View >
                      
                    <View  >
                              <Item style={{backgroundColor:'white'}} regular>
                                <Icon name="ios-search" />
                                <Input placeholder="Search"  onChangeText={(text)=>{this.setState({val:text})}}/>
                                <Button style={{height:"100%",}}
                                onPress={this.search}
                                >
                              <Text style={{color:"#fff",}}>Search</Text>
                            </Button>
                              </Item>
                            </View>


                  <View style={{flexDirection:"row",justifyContent:'center'}}>
                      
                        <Text style={{fontWeight: "bold",fontSize:25,color:"#678a01",textDecorationLine:'underline',padding:10}}>PRODUCTS</Text>
                        </View>
                        {isLoading==true && <Spinner  style={{position:'absolute',marginTop:'14%',marginLeft:"35%"}}/>}
                   </View>


          <ScrollView>

          {s_data && s_data.map((itm)=>{
                            console.warn(itm.content.rendered)
                            return <View>     
                          <View style={{marginTop:'2%',borderWidth:3,borderColor:'gray',borderRadius:10}}>
                          <Image source={{uri: 'data:image/jpeg;base64,'}} style={{height:150, width:"100%", flex: 1}}/>

                            <Text style={styles.text}>Description: {itm.content.rendered} </Text>
                            <Text style={styles.text1}>Price: {itm.price}euro</Text>
                            <View style={{position:'relative'}}>       
                            <Button style={{justifyContent:"center", width:'100%', bottom: 1,alignSelf:"center",backgroundColor:"green"}} transparent>
                              <Text style={{color:"#fff"}} >Click Here to Buy </Text>
                              </Button>
                          </View >
                          </View>

                          </View>
                          })}

          </ScrollView>


          </Card>

         

          </ScrollView>
        </Container>
        )
    }
}
const styles=StyleSheet.create({
  text:{
    color:'white',
    marginLeft:'4%',
    marginTop:'4%',
    fontWeight:'bold'
  },
  text1:{
    color:'white',
    marginTop:'4%',
    marginLeft:'4%',
    marginBottom:'5%',
    fontWeight:'bold'
  }
})
